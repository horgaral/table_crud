...
class hemeroteca extends \core\Controlador {
	...
	public function index(array $datos=array()) {
		...
		$datos["filas"] = \modelos\Datos_SQL::table("hemeroteca")->select( $clausulas ); // Recupera todas las filas ordenadas
...
*/
		...
		if ( ! $validacion = ! \core\Validaciones::errores_validacion_request($validaciones, $datos))
			...
			if ( ! $validacion = \modelos\Modelo_SQL::insert($datos["values"], 'hemeroteca')) // Devuelve true o false
				$datos["errores"]["errores_validacion"]="No se han podido grabar los datos en la bd.";
		}
		if ( ! $validacion) //Devolvemos el formulario para que lo intente corregir de nuevo
			\core\Distribuidor::cargar_controlador('hemeroteca', 'form_insertar', $datos);
		else
		{
			...
			//\core\Distribuidor::cargar_controlador('hemeroteca', 'index', $datos);
			$_SESSION["alerta"] = "Se han grabado correctamente los detalles";
			//header("Location: ".\core\URL::generar("hemeroteca/index"));
			\core\HTTP_Respuesta::set_header_line("location", \core\URL::generar("hemeroteca/index"));
...
*/	
	public function form_modificar(array $datos=array()) {
		...
		if ( ! count($datos)) { // Si no es un reenvío desde una validación fallida
			$validaciones=array(
				"id" => "errores_requerido && errores_numero_entero_positivo && errores_referencia:id/hemeroteca/id"
			);
			if ( ! $validacion = ! \core\Validaciones::errores_validacion_request($validaciones, $datos)) {
				$datos['mensaje'] = 'Datos erróneos para identificar la revista a modificar';
				$datos['url_continuar'] = \core\URL::generar("hemeroteca");
				...
				if ( ! $filas = \modelos\Datos_SQL::table("hemeroteca")->select($clausulas)) {
...
*/       
	public function validar_form_modificar(array $datos=array()) {	
		...
		if ( ! $validacion = ! \core\Validaciones::errores_validacion_request($validaciones, $datos)) {
			...
			if ( ! $validacion = \modelos\Datos_SQL::table("hemeroteca")->update_row($datos["values"], 'hemeroteca')); // Devuelve true o false
				$datos["errores"]["errores_validacion"]="No se han podido grabar los datos en la bd.";
		}
		if ( ! $validacion) //Devolvemos el formulario para que lo intente corregir de nuevo
			$this->cargar_controlador('hemeroteca', 'form_modificar',$datos);
		else
		{
			$datos = array("alerta" => "Se han modificado correctamente.");
			// Definir el controlador que responderá después de la inserción
			$this->cargar_controlador('hemeroteca', 'index',$datos);		
		}
	}
	...
	public function form_borrar(array $datos=array()) {
		...
		if ( ! $validacion = ! \core\Validaciones::errores_validacion_request($validaciones, $datos)) {
			...
			$datos['url_continuar'] = \core\URL::http('?menu=hemeroteca');
			\core\Distribuidor::cargar_controlador('mensajes', 'mensaje', $datos);
			return;
		}
		else {
			$clausulas['where'] = " id = {$datos['values']['id']} ";
			if ( ! $filas = \modelos\Datos_SQL::select( $clausulas, 'hemeroteca')) {
				$datos['mensaje'] = 'Error al recuperar la fila de la base de datos';
				\core\Distribuidor::cargar_controlador('mensajes', 'mensaje', $datos);
				return;
			}
 ...
 /**
	 * 
	 * Valida que el id sea correcto y exista. En caso de aceptar, ejecuta \modelos\Datos_SQL::delete($datos["values"], 'hemeroteca')
         * 
         * 
         * @param array $datos
*/   	
	...
	public function validar_form_borrar(array $datos=array()) {	
                ...
                if ( ! $validacion = ! \core\Validaciones::errores_validacion_request($validaciones, $datos)) {
                    ...
                    {
                    if ( ! $validacion = \modelos\Datos_SQL::delete_row($datos["values"], 'hemeroteca')) {// Devuelve true o false
