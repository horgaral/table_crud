...
class Conversiones {
	...
	public static function fecha_hora_es_a_mysql($fecha_hora_es) {
		...
		if (preg_match($patron_fecha_hora, $fecha_hora_es)) {
         ...
         /**
	 * Convierte una fecha en formato mysql a formato europeo (aaaa-mm-dd => dd-mm-aaaa)
	 * 
	 * @param string $fecha_hora_es
	 * @return string
	 */ 
		public static function fecha_mysql_a_es($fecha_mysql, $trozos = array()) {
                        $trozos = explode("-", $fecha_mysql);
...
//			return($fecha->format("d/m/Y"));
        ...
        /**
	 * Convierte una fecha en formato europeo a formato mysql (dd-mm-aaaa => aaaa-mm-dd)
	 * 
	 * @param string $fecha_hora_mysql
	 * @return string
	 */
		public static function fecha_es_a_mysql($fecha_es, $trozos = array()) {
               
                    $fecha_es = str_replace(array(' ', '-', '.', ',', ':'), '/', $fecha_es);
                    $trozos = explode("/", $fecha_es);
                    return $trozos[2]."/".$trozos[1]."/".$trozos[0];
                         
//			$fecha = \DateTime::createFromFormat("d/m/Y", $fecha_es);
